...
stages:
  bronze_to_silver:
    ...
    deps:
      ...
      - data/bronze/              # your raw data files
    outs:
      - data/silver/              # cleaned outputs
...
def bronze_to_silver():
    ...
    clean_data("raw_data.csv", "cleaned_data.parquet")
```
...
def silver_to_gold():
    ...
    create_features("cleaned_data.parquet", "features.parquet")
```
...
stages:
  bronze_to_silver:
    ...
    deps:
      - src/pipelines/data/bronze_to_silver.py
      - data/bronze/raw_data.csv             # Add your input files
    outs:
      - data/silver/cleaned_data.parquet     # Add your output files

  silver_to_gold:
    cmd: uv run python -m src.pipelines.data.silver_to_gold
    deps:
      - src/pipelines/data/silver_to_gold.py
      - data/silver/cleaned_data.parquet     # Depends on previous stage output
    outs:
      - data/gold/features.parquet           # Add your output files
```
...
Create a new file in `src/pipelines/models/` (e.g., `my_model.py`):
...
## features.parquet
- Created: YYYY-MM-DD
- Source: data/silver/cleaned_data.parquet
- Processing: src/pipelines/data/silver_to_gold.py
- Features: N columns including time-based features
- Rows: N
```
