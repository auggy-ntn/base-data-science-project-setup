...
**This guide helps the project owner set up the infrastructure so the team can collaborate effectively.**
...
2. Create Object Storage container:
   ...
   - Container name: `my-project` (or your choice)
...
3. Create S3 credentials:
   ...
   - **SAVE THESE CREDENTIALS** (shown only once):
     - `Access Key` → This is your `DATA_ACCESS_KEY_ID`
     - `Secret Key` → This is your `DATA_SECRET_ACCESS_KEY`
...
# Add OVH S3 remote
dvc remote add -d ovh-storage s3://my-project
...
```bash
# Add to .env
echo "DATA_ACCESS_KEY_ID=your_access_key_here" >> .env
echo "DATA_SECRET_ACCESS_KEY=your_secret_key_here" >> .env
...
```bash
dvc remote add -d s3remote s3://my-project/dvc-storage
...
3. **Create an MLflow experiment:**
   ...
   - Name it: `my-project` (or your choice)
...
# .env.example
# DVC Remote Storage (OVH Object Storage)
DATA_ACCESS_KEY_ID=your_access_key_from_owner
DATA_SECRET_ACCESS_KEY=your_secret_key_from_owner
```
